...
const requireProfessionalOrAdmin = async (
  req: Request,
  res: Response,
  next: any,
) => {
  ...
  if (!allowedRoles.includes(foundUser.role)) {
    return res.status(403).json({
      success: false,
      message:
        "Acceso denegado. Se requieren privilegios de profesional o administrador",
    });
  }
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      if (currentUser.role === "admin") {
        ...
        ["teacher", "nutritionist", "psychologist"].includes(currentUser.role)
      ) {
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      const isProfessional =
        clase.profesionalId.toString() === currentUser?._id.toString();
      const isStudent =
        clase.alumnoId.toString() === currentUser?._id.toString();
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      if (currentUser?.role === "admin") {
        ...
        filter.$or = [{ tipo: "global" }, { tipo: "profesional" }];
      }
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      const nuevoBloqueo = new Bloqueo({
        ...
        profesionalId:
          tipo === "global" ? undefined : profesionalId || currentUser._id,
...
router.delete(
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      const isAdmin = currentUser.role === "admin";
      const isOwner =
        bloqueo.profesionalId?.toString() === currentUser._id.toString();
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      const plan = await PlanUsuario.findOne({ userId: targetUserId })
        ...
        .populate(
          "historial.profesionalId",
          "firstName lastName role specialty",
        )
...
router.get(
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      const [clasesHoy, clasesSemana, clasesTotal, clasesCompletadas] =
        await Promise.all([
      ...
      res.json({
        ...
        data: {
          ...
          tasaCompletacion:
            clasesTotal > 0
              ? Math.round((clasesCompletadas / clasesTotal) * 100)
              : 0,
        },
      });
    } catch (error) {
